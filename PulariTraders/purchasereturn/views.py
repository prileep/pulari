...
def purchasereturn(request, rid=None):
    ...
    if request.method == "POST" and request.POST.get("action") == "cancel":
        with transaction.atomic():
            ...
            rid = request.POST.get("pr_rid") or request.POST.get("rid") or rid

            PurchasereturnHeader.objects.filter(pr_rid=rid).update(
                pr_status="Cancelled"
            )

            with connection.cursor() as cursor:
                # FIXED: Changed from purchase_return_header.pr_rid (which was None) to the direct local 'rid' variable
                cursor.callproc('post_purchase_return', [rid])
